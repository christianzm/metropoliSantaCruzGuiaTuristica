...
const createCustomMarkerIcon = (category: string, color: string, isSelected: boolean = false) => {
...
export default function ExplorePage() {
  ...
  // Use custom hooks
  const { data: cacheData, loading: cacheLoading, error: cacheError, loadData, refreshData } = useDataCache()
  const { location, permission, error: locationError, loading: locationLoading, requestLocation } = useGeolocation()
 ...
 const handlePlaceSelect = async (place: PlaceWithDetails) => {
  ...
  useEffect(() => {
    setSelectedPlace(null)
  }, [filters.searchTerm, filters.selectedCategory, filters.selectedMunicipality])

  // Handle place click and record click
  const handlePlaceClick = async (place: PlaceWithDetails, clickType: string) => {
  ...
  if (cacheError) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">{t.errorLoadingData}</h1>
  ...
  <Link href="/" className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-2">
            ...
            <div className="flex min-w-0 shrink-0 items-center gap-1 sm:gap-3">
              <Button variant="outline" size="sm" onClick={() => setShowFilters(!showFilters)} className="h-9 w-9 shrink-0 px-0 sm:w-auto sm:px-3 md:hidden" aria-label="Filtros">
                <Filter className="h-4 w-4 sm:mr-2" />
                <span className="hidden sm:inline">{t.filters}</span>
                
              </Button>

              <Button onClick={refreshData} size="sm" variant="outline" disabled={cacheLoading}>
                <RefreshCw className={`w-4 h-4 mr-2 ${cacheLoading ? "animate-spin" : ""}`} />
              ...
              {permission === "denied" && (
                <Button onClick={requestLocation} size="sm" className="bg-green-600 hover:bg-green-700">
      ...
      <div className="flex h-[calc(100vh-4rem)]">
        ...
        <div
          className={`w-full md:w-96 bg-white border-r overflow-y-auto ${showFilters ? "block" : "hidden md:block"}`}
        >
          <div className="p-4 border-b">
            ...
            <div className="mb-4">
              ...
              <Select value={filters.selectedCategory} onValueChange={updateCategory}>
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  {cacheData?.categories.map((category) => (
                    <SelectItem key={category.id} value={category.id}>
                      <div className="flex items-center">
                        <span className="mr-2 text-sm">{categoryIcons[category.name] || "📍"}</span>
                        <div
            ...
            {/* Municipality Filter */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">{t.municipality}</label>
              <Select value={filters.selectedMunicipality} onValueChange={updateMunicipality}>
                <SelectTrigger>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <Button
                  variant={sortOptions.field === "rating" ? "default" : "outline"}
                  size="sm"
                ...
                >
                  ...
                  {t.rating}
                  {sortOptions.field === "rating" && <ArrowUpDown className="w-3 h-3 ml-1" />}
                </Button>
                <Button
                ...
                >
                  {t.name}
                  {sortOptions.field === "name" && <ArrowUpDown className="w-3 h-3 ml-1" />}
                </Button>
                {permission === "granted" && (
                  <Button
                    variant={sortOptions.field === "distance" ? "default" : "outline"}
                    size="sm"
                  ...
                  >
                    ...
                    {t.distance}
                    {sortOptions.field === "distance" && <ArrowUpDown className="w-3 h-3 ml-1" />}
                  </Button>
                )}
              </div>
            </div>
          ...
          <div className="p-4 space-y-4">
            {filteredPlaces.map((place) => {
              ...
              const categoryIcon = primaryCategory ? categoryIcons[primaryCategory.name] : "📍"
              
              return (
                <Link key={place.id} href={`/place/${place.id}`} onClick={() => handlePlaceClick(place, "detail_view")}>
                  <Card
                    className={`cursor-pointer transition-all hover:shadow-md ${
                      selectedPlace?.id === place.id ? "ring-2 ring-green-500" : ""
                    }`}
                    ...
                    <CardContent className="p-4">
                      <div className="flex space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-gray-900 truncate">{place.commercial_name}</h3>
                          ...
                          <div className="flex items-center justify-between mt-2">
                            <div className="flex items-center space-x-2 text-xs text-gray-500">
                              ...
                              {permission === "granted" && place.distance !== undefined && (
                                <div className="flex items-center">
                            ...
                            {primaryCategory && (
                              <Badge
                                style={{
                                  backgroundColor: categoryColors[primaryCategory.name] || "#6B7280",
                                }}
        ...
        {/* Map */}
        <div className={`flex-1 relative ${showFilters ? "hidden md:block" : "block"}`}>
          <MapContainer
          ...
          >
            ...
            {permission === "granted" && (
              <>
                <Marker 
                  ...
                  icon={createCustomMarkerIcon("user-location", "#3B82F6")}
                >
           ...
           {/* Place markers - MODIFICADO */}
            {filteredPlaces.map((place) => {
              const primaryCategory = getPrimaryCategory(place)
              const categoryColor = primaryCategory ? categoryColors[primaryCategory.name] || "#6B7280" : "#6B7280"
              ...
              return (
                <Marker
                  ...
                  icon={createCustomMarkerIcon(categoryName, categoryColor, isSelected)}
                  eventHandlers={{
                    click: () => {
                      handlePlaceSelect(place)
                    },
                    popupclose: () => {
                      // Limpiar selectedPlace cuando se cierra el popup
                ...
                >
                  ...
                  >
                    <div className="w-64">
                      ...
                      <h3 className="font-semibold text-lg mb-1">{place.commercial_name}</h3>
                      ...
                      <Link href={`/place/${place.id}`} onClick={() => handlePlaceClick(place, "popup_detail")}>
                        <Button 
                          size="sm" 
         ...
         {/* Selected Place Details - MODIFICADO con mejor z-index y posicionamiento 
          {selectedPlace && (
            <div 
              className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-white rounded-lg shadow-2xl p-4 max-w-sm w-11/12 md:w-96 border border-gray-200"
              style={{ 
                zIndex: 10000,
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <span className="text-xl flex-shrink-0">
                    {categoryIcons[getPrimaryCategory(selectedPlace)?.name || "default"] || "📍"}
                  </span>
                  <h3 className="font-semibold text-lg truncate">{selectedPlace.commercial_name}</h3>
                </div>
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={handleCloseSelectedPlace}
                  className="flex-shrink-0 ml-2 hover:bg-gray-100 rounded-full w-8 h-8 p-0"
                >
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              <div className="flex items-center justify-between text-sm mb-3">
                <div className="flex items-center">
                  <Star className="w-4 h-4 text-yellow-500 mr-1" />
                  {selectedPlace.rating_avg?.toFixed(1) || t.na}
                </div>
                <div className="flex items-center">
                  <MapPin className="w-4 h-4 text-gray-500 mr-1" />
                  {selectedPlace.municipality}
                </div>
                {permission === "granted" && selectedPlace.distance !== undefined && (
                  <div className="flex items-center">
                    <Navigation className="w-4 h-4 text-gray-500 mr-1" />
                    {selectedPlace.distance.toFixed(1)} {t.km}
                  </div>
                )}
              </div>
              ...
              <Link
                ...
                onClick={() => handlePlaceClick(selectedPlace, "bottom_panel_detail")}
              >
                <Button
                  className="w-full text-white flex items-center gap-2 transition-all hover:shadow-lg"
                  style={{
                    backgroundColor: getPrimaryCategory(selectedPlace)
                      ? categoryColors[getPrimaryCategory(selectedPlace)!.name] || "#6B7280"
                      : "#6B7280",
                  }}
                >
                  <span>
